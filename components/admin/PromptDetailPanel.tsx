...
import { adminApprovePrompt, adminRejectPrompt } from "@/app/admin/review/actions";
...
export async function PromptDetailPanel({
  ...
  const canApprove = prompt.moderationStatus === "PENDING" && typeof score === "number" && score >= 50;
  const isPublished = prompt.status === "PUBLISHED" && prompt.moderationStatus === "APPROVED";

  return (
    <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-black/10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-semibold text-gray-900">{prompt.user.email}</div>
            ...
            <>
              <form action={adminApprovePrompt}>
                ...
                <button
                  ...
                  title={canApprove ? "Approve and publish" : "Requires score ≥ 50"}
                >
                  Approve & publish
                </button>
              </form>
      ...
      {prompt.analysis ? (
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          ...
          <div className="rounded-lg bg-white p-3 ring-1 ring-black/10">
            ...
            <div className="mt-0.5 text-xs text-gray-800 whitespace-pre-wrap">
